...
{
    return ((m_sExeFile.size() + sizeof(uint32_t)) + (m_sID.size() + sizeof(uint32_t)) + sizeof(m_taskIndex) +
            sizeof(m_collectionIndex) + (m_taskPath.size() + sizeof(uint32_t)) +
            (m_groupName.size() + sizeof(uint32_t)) + (m_collectionName.size() + sizeof(uint32_t)) +
            (m_taskName.size() + sizeof(uint32_t)));
}
