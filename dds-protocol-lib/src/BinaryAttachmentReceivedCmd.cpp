...
{
    return (m_receivedFilePath.size() + sizeof(uint32_t)) + (m_requestedFileName.size() + sizeof(uint32_t)) +
           sizeof(m_srcCommand) + sizeof(m_receivedFileSize) + sizeof(m_downloadTime);
}
