...
namespace dds
{
    namespace protocol_api
    {
        ...
        cmdUPDATE_KEY,     // attachment: SUpdateKeyCmd
            ...
            cmdGET_PROP_LIST, cmdGET_PROP_VALUES,
        cmdPROGRESS, // attachment: SProgressCmd
            cmdWATCHDOG_HEARTBEAT,
        cmdSET_TOPOLOGY,         // attachment: SSetTopologyCmd
            cmdSET_MASTER_AGENT, // attachment: SHostInfoCmd
            /// this command contains a number of connected agents to a given master agent
            cmdAGENT_TEAM_SIZE // attachment: SUint32Cmd
    };
    ...
    static std::map<uint16_t, std::string> g_cmdToString{
        ...
        { cmdSET_MASTER_AGENT, NAME_TO_STRING(cmdSET_MASTER_AGENT) },
        { cmdAGENT_TEAM_SIZE, NAME_TO_STRING(cmdAGENT_TEAM_SIZE) }
    };
}
